...
class UserDao:
    ...
    def login(self,username,password):
        try:
            ...
            cursor.execute(sql,[username,password])#两个参数是元组也可以是列表
    ...
    def search_user_role(self,username):
        try:
            ...
            sql="SELECT r.role FROM t_user u JOIN t_role r ON u.role_id=r.id " \
                "WHERE u.username=%s"
            cursor.execute(sql,[username])#必须是列表
    ...
    def update(self,id,username,passwprd,email,role_id):
        try:
            ...
            sql = "UPDATE t_user SET username=%s, " \
                  ...
                  "email=%s,role_id=%s " \
                  "WHERE id =%s"
            cursor.execute(sql, (username,passwprd,email,role_id,id))
            con.commit()
        except Exception as e:
            if "con" in dir():
                con.rollback()
